...
var dirSnapshots = map[string]string{}

// dirMightHaveChanged compares every JSON filename, mtime, and size against
// the last-seen snapshot. First call seeds the snapshot and returns false.
func dirMightHaveChanged(dir string) bool {
	cur := scanSnapshot(dir)
	prev, seen := dirSnapshots[dir]
	dirSnapshots[dir] = cur
	return seen && prev != cur
}

func scanSnapshot(dir string) string {
...
func (m *model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case dirChangedMsg:
		...
		if msg.changed {
			// A file was added/edited: reload and keep the cursor valid. Capture
			// the applied name, not the picker cursor, so editing an active theme
			// still repaints after the cursor has moved elsewhere.
			active := theme.Active()
			m.themes = reloadThemes(m.dir)
			m.cursor = clamp(m.cursor, 0, len(m.themes)-1)
			if _, ok := theme.Find(m.themes, active); ok {
				note := theme.Apply(active, nil)
				m.setViewport()
				m.flash(note)
			}
		}
