...
func main() {
	...
	experimental := cfg.Experimental["hashlineEdit"]
	if *benchFlag {
		...
		_ = prov.Key()
		ag := agent.New(llm.New(prov.BaseURL, "bench"), id, mdl.MaxTokens, systemPromptWithExperimental(experimental))
		if experimental {
			ag.UseHashlineTools()
		}
		return
	...
	sessionID, err := tui.Run(cfg, *modelFlag, *providerFlag, systemPromptWithExperimental(experimental), *resumeFlag, *cautiousFlag)
	if err != nil {
		fmt.Fprintln(os.Stderr, "whip:", err)
		os.Exit(1)
	}
