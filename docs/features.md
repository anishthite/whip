...
- **`whip up <prompt>`** is a shell shorthand for a one-shot `whip run`.
  It joins every argument after `up` into the prompt, including words that
  begin with `-`; normal shell quoting applies. Tests: `up_test.go`.
