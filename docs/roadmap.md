...
- [x] Terminal window title: `whip <cwd>` while running — bubbletea's `SetWindowTitle` emits OSC 2; re-emitted on `/cd` so the title tracks the current directory. The shell reclaims the title at its next prompt after exit, so no restore sequence is needed.
...
- [ ] Spinner with elapsed time + token count (% of context window) in status line (opencode `routes/session/footer.tsx`) — cost part done (status line shows session spend when the provider advertises pricing)
...
- [x] Live tokens-per-second gauge in the status line — estimates streaming completion throughput over a one-second window; `tpsGauge` selects tach (default), bar, spark, lights, or off; `cmd/tps-demo` is the interactive visual lab
...
- [x] Codex subscription provider: `auth:"codex"` reuses local Pi/Codex CLI OAuth, refreshes expiring credentials, and maps the ChatGPT Codex Responses SSE endpoint into the existing tool loop; configured context/maxOut stay authoritative because it has no compatible `/models` catalog
