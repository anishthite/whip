...
func isKnownRole(r Role) bool { return slices.Contains(allRoles, r) }
...
func Apply(name string, detect DetectFunc) string {
	...
	if bg == BgAuto {
		...
		SetUnknownMarkdown()
		return t.Name + " (undetermined — neutral default)"
	}
	mu.Lock()
	active = t.Name
...
func parseFile(path, name string) (Theme, error) {
	raw, err := os.ReadFile(path) //nolint:gosec // G304: theme paths come only from the user's theme directory
	if err != nil {
		return Theme{}, err
	}
...
func buildMdStyle(known, light bool) glamouransi.StyleConfig {
	...
	if light {
		...
		st.Code.Color = new("124")           // dark red text
